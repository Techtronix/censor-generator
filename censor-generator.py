...
def main():
    ...
    print('[config] Using ' + args.output + ' as output file')
    print('[config] Using ' + args.replace + ' as replacement string for censored words')
    print('[config] Using ' + args.url + ' as wordlist source')

    # This is actually the meat and potatoes
    response = requests.get(args.url)
    output = open(args.output, 'w')
    output.write('# censor.conf auto-generated by censor-generator.py\n')
    output.write('# Source availalbe at https://www.techtronix.net\n')
    for word in response.iter_lines(decode_unicode=True):
        line = '<badword text="' + word + '" replace="' + args.replace + '">\n'
        output.write(line)
    output.close()
